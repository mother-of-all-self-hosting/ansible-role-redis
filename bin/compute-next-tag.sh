...
#
# Tags look like `v<Redis version>-<release>`:
#
# - if defaults/main.yml points at a Redis version that has never been
#   released, the release counter restarts at 0 (`v8.10.1-0`)
# - otherwise the counter is incremented (`v8.10.1-1`), but only if something
...
version="$(sed -nE 's|^redis_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Redis version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `8.10.1`), but the tags
# do (`v8.10.1-0`). Stripping any `v` before prepending one keeps this correct
